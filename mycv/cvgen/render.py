...
def render_cv_tex(
    config: CVConfigRoot,
    active_section_filenames: list[str],
    env: Environment | None = None,
    *,
    data: ProfessionalData | None = None,
) -> str:
    """Render the top-level cv.tex from the template."""
    if env is None:
        env = _make_env()
    tmpl = env.get_template("cv.tex.j2")
    personal_info = data.personal_info if data is not None else None
    return tmpl.render(
        citations_mode=config.citations.mode,
        active_sections=active_section_filenames,
...
def render_all(
    ...
    for section_name in SECTION_TITLES:
        ...
        active_section_filenames.append(filename)
    ...
    active_section_filenames.append("citations.tex")
    ...
    # Generate cv.tex in the parent directory (alongside cv/ output dir)
    cv_tex_content = render_cv_tex(config, active_section_filenames, env, data=data)
    cv_tex_path = output_dir.parent / "cv.tex"
    _ = cv_tex_path.write_text(cv_tex_content)
    generated.append(cv_tex_path)
